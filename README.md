...
<body>
    ...
    <script>
        ...
        const tanggalUltah = new Date("July 04, 2026 18:36:00").getTime();
        ...
        function mulaiGameAtauProteksi() {
            initAudioContext(); // Buka jalur audio saat klik surat
            const sekarang = new Date().getTime();
            if (sekarang < tanggalUltah) {
                alert("Eitss, mauu bukaa suratnyaa yaaa zaraaaaa?wkwkwkwkwkw, belumm waktunyaa zaraaaaa! 😜 Sabarr duluu yaaa suratnyaa masihh dikuncii, tungguu sampaii nantii malamm tanggal 5 Juli baruu bisa dibukaa!hehehehehe 🤞🏻❤️");
            } else {
                document.getElementById("tombol-memicu-game").style.display = "none";
                document.getElementById("balloon-area").style.display = "flex";
                alert("Spesial umur zaraaa yang ke-21, yuk pecahin duluu 21 balon di bawah ini buat buka suratnya! 🎈💥");
            }
        }

        function pecahkanBalon(elemenBalon) {
            // Mainkan suara letusan internal (tanpa download, anti delay)
            bunyikanPopInternal();
            ...
            if (jumlahBalonPecah >= totalBalon) {
                ...
                alert("Yeeay! 21 Balon sudah dipecahkan! Coba lihat, kata-kata pujian buat zaraaa langsung berjatuhan! Kapsul kado dan tombol rahasia juga udah kebuka di bawah! ✨💖");
            }
        }
        ...
        function klaimVoucherLangsung(elemenKupon, jenisVoucher) {
            ...
            } else if (jenisVoucher === 'Quality Time') {
                alert("🎟️ Voucher Quality Time BERHASIL DIKLAIM!\n\nAsyiik, siap-siap luangkan waktu seharian full buat ngabisin waktu berdua bareng Uqii yaa! 🥰✨");
            } else if (jenisVoucher === 'Surprise Date') {
                alert("🎟️ Voucher Surprise Date BERHASIL DIKLAIM!\n\nTunggu tanggal mainnya yaa! Uqii bakal siapin kejutan buat nge-date terspesial khusus buat Zaraaa! 🕵️‍♂️❤️");
            } else if (jenisVoucher === 'Jalan-Jalan') {
                alert("🎟️ Voucher Jalan-Jalan BERHASIL DIKLAIM!\n\nSiap-siap tentukan rute jalan-jalan selanjutnya, Uqii bakal jadi supir setiamu! 🚗💨");
            }
        }

        const teksBuzzer = [
            "Uqii juga kangen bangeeet sama Zaraaa! ❤️",
            "Ayo keluarr lagii zaraaaa, kitaa ketemuann lagii, mainn lagii zaraaaa! 🫶🏻",
            "Hehehe kangen yaaa? Sini zaraaaaaa! 🤗",
            "Kangennya dikirim ke WA dong, jangan cuma dipencet! 😜",
        ];
        
        function pencetBuzzer() {
            // Bunyikan instan suara buzzer imut
            bunyikanBuzzerInternal();
        ...
        function putarMusik() {
            initAudioContext(); // Buka izin audio context browser
